...
func updateGradeConsumer(ctx context.Context) {
	cfg := shared.LoadConfig("config/create_grade_consumer.yaml")
	kafkaCfg := pkg.NewKafkaConsumerConfig()

	consumer, err := sarama.NewConsumerGroup([]string{cfg.Kafka.Broker}, leaderboard.GradeUpdateConsumerGroup, kafkaCfg)
	if err != nil {
	...
	go func() {
		for err = range consumer.Errors() {
			log.Printf("consumer error, topic %s, error %s", leaderboard.UpdateGradeTopic, err.Error())
		}
	}()
	...
	go func() {
		for {
			...
			case <-newCtx.Done():
				...
				return
			default:
				err = consumer.Consume(newCtx, []string{leaderboard.UpdateGradeTopic},
					pkg.NewKafkaConsumer(&leaderboard.UpdateGradeHandler{}, 1000),
				)
				if err != nil {
					log.Printf("consume message error, topic %s, error %s", leaderboard.UpdateGradeTopic, err.Error())
					return
				}
			}
		}
	...
	log.Printf("consumer up and running, topic %s, group: %s", leaderboard.UpdateGradeTopic, leaderboard.GradeSubmissionConsumerGroup)
