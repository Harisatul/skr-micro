...
func createAnswerConsumer(ctx context.Context) {
	cfg := shared.LoadConfig("config/create_submission_consumer.yaml")
	kafkaCfg := pkg.NewKafkaConsumerConfig()

	consumer, err := sarama.NewConsumerGroup([]string{cfg.Kafka.Broker}, grade.GradeAnswerConsumerGroup, kafkaCfg)
	if err != nil {
	...
	go func() {
		for err = range consumer.Errors() {
			log.Printf("consumer error, topic %s, error %s", grade.CreateAnswerTopic, err.Error())
		}
	}()
	...
	go func() {
		for {
			...
			case <-newCtx.Done():
				...
				return
			default:
				err = consumer.Consume(newCtx, []string{grade.CreateAnswerTopic},
					pkg.NewKafkaConsumer(&grade.CreateAnswerHandler{}, 1000),
				)
				if err != nil {
					log.Printf("consume message error, topic %s, error %s", grade.CreateAnswerTopic, err.Error())
					return
				}
			}
		}
	...
	log.Printf("consumer up and running, topic %s, group: %s", grade.CreateAnswerTopic, grade.GradeAnswerConsumerGroup)
